...
class SecurityRateLimitController extends Controller
{
    ...
    public function disableUsers()
    {
        ...
        $total = TokenPoint::whereIn('user_id', $userIds)->sum('amount');

        return response()->json(['user_ids'=>$total]);


        foreach (array_chunk($userIds,400) as $ids){
            $ipAddresses = User::whereIn('id', $ids)->pluck('last_active_from_ip')->toArray();
            ...
            TokenPoint::whereIn('user_id', $ids)->whereNull('claimable_at')->update(['claimable_at' => TokenPoint::fromDateTime(Carbon::now()), 'claimable_by' => 'security.rate_limit']);
            User::whereIn('id', $ids)->update(['status' => User::STATUS_INACTIVE]);
        }
