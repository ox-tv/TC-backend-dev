...
class VideoStatisticsController extends Controller
{
    ...
    public function videoStatisticsOverview(Request $request, $idOrHash)
    {
        ...
        return response()->json([
            ...
            'likes_hero' => ($temp = $statisticsQuery->sum('likes_hero')) > 0? $temp : 0,
            'likes_non_hero' => ($temp = $statisticsQuery->sum('likes_non_hero')) > 0? $temp : 0,
            'likes_total' => ($temp = $statisticsQuery->sum('likes_total')) > 0? $temp : 0,
            'dislikes_hero' => ($temp = $statisticsQuery->sum('dislikes_hero')) > 0? $temp : 0,
            'dislikes_non_hero' => ($temp = $statisticsQuery->sum('dislikes_non_hero')) > 0? $temp : 0,
            'dislikes_total' => ($temp = $statisticsQuery->sum('dislikes_total')) > 0? $temp : 0,
